...
def setup():
    ...
    class DifferentialMeshGrowth:
        ...
        def __init__(self, steps, v_in, e_in, f_in, w_in, max_edge_length, relax_iterations, boundary_list):
            ...
            datas = match_long_repeat([v_in, e_in, f_in, w_in, boundary_list])
            for v_datas, e_datas, f_datas, w_datas, b_datas in zip(*datas):
                
                vertices = np.array(v_datas)
                edges = np.array(e_datas)
                faces = np.array(f_datas)
                weights = np.array(w_datas)
                boundaries = np.array(b_datas)
                ...
                for step in range(self.step_num):
                    ...
                    vertices, edges, faces, velocities, weights = self.process(vertices, edges, faces, velocities, weights, boundaries)
        ...
        def process(self, vertices, edges, faces, velocities, weights, boundaries):
            
            vertices = self.relax_mesh(vertices, edges, faces, weights)

            vertices, edges, faces = self.subdivide_long_edges(vertices, edges, faces)
            
            velocities, weights = self.compute_velocities(vertices, edges, velocities, weights, boundaries)
            vertices += velocities
        ...
        def compute_velocities(self, vertices, edges, velocities, weights, boundaries):
            ...
            border = np.zeros((vertices_num, 3))
            ...
            for i in range(vertices_num):
                ...
                if len(boundaries) > 0:
                    ...
                    boundary_distance = np.linalg.norm(boundary_array - vertices[i], axis=1)
                ...
                border[i] = self.BOUNDARY_FORCE * (np.sum(vertices[i] - boundary_agents, axis=0)) if len(boundary_agents) > 0 else 0

            weight_vec = np.repeat(weights[:, None], 3, axis=1) + 0.01
            if self.is_first:
                velocities += (cohesion + separation + border) * weight_vec
            else:
                velocities = (cohesion + separation + border) * weight_vec
    ...
    DFMG = DifferentialMeshGrowth(steps, vertices_in, edges_in, faces_in, weights_in, max_edge_length_in, relax_iterations_in, verts_boundary)
